...
function InvoicePreview({ d }) {
  ...
  const grand = amt + exp;
  const due = d.customDue
    ? fmtDate(new Date(d.customDue + "T00:00:00"))
    : fmtDate(addDays(today, Number(d.paymentTermsDays) || 30));
    ...
    <div style={{ fontFamily: "Georgia, serif", background: "#fff", color: "#222", padding: "28px 32px", borderRadius: 8, fontSize: 12, boxShadow: "0 4px 24px rgba(0,0,0,0.13)" }}>
      ...
      <div style={{ background: "#f0f4ff", borderLeft: "4px solid #1a1a2e", padding: "12px 16px", fontSize: 11, borderRadius: "0 6px 6px 0" }}>
        ...
        <div style={{ marginTop: 8, color: "#666", fontStyle: "italic" }}>
          * Reference invoice number when sending payment.<br />
          Questions: {AIVC.name} | {AIVC.phone} | {AIVC.email}
        </div>
      </div>
    </div>
  );
...
export default function InvoiceDashboard() {
  ...
  async function generatePDF() {
    ...
    addLog("Calling /api/generate-pdf...");
    try {
      const invoiceData = {
        invoice_num: d.invoiceNum, phase: d.phase,
        issue_date: fmtDate(today), due_date: dueDate,
        from_name: AIVC.name, from_company: AIVC.company,
        from_address: "117 W 17th St New York, NY 10011",
        from_email: AIVC.email, from_ein: AIVC.ein, from_phone: AIVC.phone,
        bill_to_name: d.billToName || "Accounts Payable",
        client_name: d.clientName, bill_to_email: d.billToEmail,
        line1_label: d.invoiceType === "first"
          ? "50% Professional Fees (SOW Commencement)"
          : "50% Professional Fees (Final Delivery)",
  ...
  const field = (label, key, type = "text", extra = {}) => (
    <div style={{ marginBottom: 14 }}>
      ...
      <input type={type} value={d[key]} onChange={set(key)} style={inputStyle} {...extra} />
    </div>
  );

  const ActionBtn = ({ onClick, loading: ld, done, color, children }) => (
    <button onClick={onClick} disabled={ld} style={{ width: "100%", padding: "13px", borderRadius: 8, border: done ? "1px solid rgba(100,220,100,0.35)" : "none", cursor: ld ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 13, letterSpacing: 0.3, transition: "all 0.2s", background: done ? "rgba(100,220,100,0.15)" : ld ? "rgba(255,255,255,0.08)" : color, color: done ? "#7cdc64" : ld ? "rgba(255,255,255,0.4)" : "#fff" }}>
      {ld ? "⏳ Working..." : done ? `✅ ${children} — Done` : children}
    </button>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(140deg,#0c0c1e 0%,#1a1040 60%,#0f1a2e 100%)", fontFamily: "'Trebuchet MS', sans-serif", color: "#fff" }}>
      <div style={{ background: "rgba(255,255,255,0.04)", borderBottom: "1px solid rgba(255,255,255,0.08)", padding: "16px 36px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 17, letterSpacing: 0.5 }}>⚡ AIVC Invoice Automation</div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>Generate & download your invoice PDF</div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          {["Details", "Preview & Send"].map((s, i) => (
            <button key={s} onClick={() => setStep(i)} style={{ padding: "7px 18px", borderRadius: 20, border: "none", cursor: "pointer", fontSize: 12, fontWeight: step === i ? 700 : 400, background: step === i ? "rgba(124,140,255,0.25)" : "rgba(255,255,255,0.06)", color: step === i ? "#a78bfa" : "rgba(255,255,255,0.5)", borderBottom: step === i ? "2px solid #a78bfa" : "2px solid transparent" }}>
              {i + 1}. {s}
            </button>
          ))}
        </div>
            ...
            <div>
              ...
              <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 10, padding: "18px 20px", marginBottom: 16 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 12px" }}>
                  ...
                  {field("Total SOW Fee ($)", "totalFee", "number")}
                </div>
                {field("Client / Company Name", "clientName")}
                {field("Phase Label", "phase")}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 12px" }}>
                  {field("Bill To Name", "billToName")}
                  {field("Bill To Email", "billToEmail", "email")}
                </div>
                {d.invoiceType === "final" && field("Expenses Amount ($)", "expenses", "number")}
              </div>
              <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 10, padding: "18px 20px", marginBottom: 16 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 12px" }}>
                  ...
                  {field("Override Due Date", "customDue", "date")}
                </div>
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", marginTop: 4 }}>
                  Due: <strong style={{ color: "#a78bfa" }}>{dueDate}</strong> · Amount: <strong style={{ color: "#a78bfa" }}>${fmt(amt)}</strong>
                  {exp > 0 && ` + $${fmt(exp)} expenses`}
                </div>
              </div>
              <button onClick={() => setStep(1)} style={{ width: "100%", padding: "13px", borderRadius: 8, border: "none", background: "linear-gradient(90deg,#7c8cff,#a78bfa)", color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 10, padding: "18px 20px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14 }}>Generate PDF</div>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)" }}>Invoice #{d.invoiceNum} · ${fmt(grand)} USD</div>
                  </div>
                </div>
              ...
              {logs.length > 0 && (
                <div style={{ background: "rgba(0,0,0,0.35)", borderRadius: 8, padding: "12px 14px", fontFamily: "monospace", fontSize: 11 }}>
                  ...
                  {logs.map((l, i) => (
                    <div key={i} style={{ color: logColor[l.type], marginBottom: 4, lineHeight: 1.5 }}>
                      <span style={{ color: "rgba(255,255,255,0.2)", marginRight: 8 }}>{l.t}</span>{l.msg}
                    </div>
                  ))}
                </div>
              )}
